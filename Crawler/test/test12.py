...
def get_html(url,porxys):
    ...
    try:
        if proxy:
            ...
            response = requests.get(url,headers=headers,timeout=3,allow_redirects=False,proxies=proxies)
        else:
